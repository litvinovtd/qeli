...
internal static class WinDivertSelfTest
{
    public static int RunUnit(Action<string, bool> check)
    {
        ...
        flows.RememberOutbound(17, client, srcA, 53001, remote1, 53, in addrA, dnsOrig);
        check("flow: DNS orig dst remembered",
            flows.TryGetInbound(17, remote1, 53, client, 53001, out var fd)
            && fd.ActiveDnsOrigDst != null && fd.ActiveDnsOrigDst.Equals(dnsOrig));
        ...
        using (var includeMap = new ProcessAppMap(Array.Empty<string>(), includeMode: true))
        {
            ...
            check("include: non-TCP/UDP is Drop",
                includeMap.Classify(1, IPAddress.Parse("127.0.0.1"), 0) == PacketDisposition.Drop);
        }
        ...
        // Filter must capture IPv6 (no trailing `and ip`).
        string filter = WinDivertAdapter.BuildFilter();
        check("filter: captures IPv6 (no `and ip`)",
            !filter.TrimEnd().EndsWith("and ip", StringComparison.Ordinal)
            && filter.Contains("ipv6.HopLimit", StringComparison.Ordinal)
