#!/usr/bin/env python3
"""Host-neutral A/B: 0.7.9 (git tag v0.7.9) vs 0.7.11 (current /opt/qeli-src),
measured BACK-TO-BACK in the same lab session so both binaries see the identical
hypervisor conditions (CPU steal cancels out). Answers "did 0.7.11 regress?"
independent of noisy-neighbor host drift.

Builds 0.7.9 in an ISOLATED tree (/opt/qeli-079) from `git archive v0.7.9` —
the 0.7.11 working tree (/opt/qeli-src) is untouched. For each wire mode we run
0.7.9 then 0.7.11 back-to-back (interleaved per mode); reality-tls is repeated
...
def build_071():
    ...
    subprocess.run(["git", "archive", "v0.7.9", "-o", tar, "--", "qeli"], cwd=REPO, check=True)
    ...
    c.close()
    if rc != 0:
        sys.exit("0.7.9 build FAILED")
    print("0.7.9 binary:", ver, sha)
    return ver, sha
...
def main():
    ...
    VARIANTS = [("0.7.9", SRC_071), ("0.7.11", SRC_079)]
    rows = {}
    for m in bm.MODES:
        name = m["name"]
        reps = RTLS_RUNS if name == "tcp-reality-tls" else 1
        acc = {"0.7.9": {"up": [], "down": []}, "0.7.11": {"up": [], "down": []}}
        for _ in range(reps):
        ...
        # live print
        if m["transport"] == "udp":
            print(f"== {name}: 0.7.9 loss400/500={acc['0.7.9'].get('loss400')}/{acc['0.7.9'].get('loss500')} "
                  f"| 0.7.11={acc['0.7.11'].get('loss400')}/{acc['0.7.11'].get('loss500')}")
        else:
            def med(xs):
                return round(statistics.median(xs), 1) if xs else None
            u1, d1 = med(acc["0.7.9"]["up"]), med(acc["0.7.9"]["down"])
            u2, d2 = med(acc["0.7.11"]["up"]), med(acc["0.7.11"]["down"])
            du = f"{(u2-u1)/u1*100:+.1f}%" if (u1 and u2) else "?"
            dd = f"{(d2-d1)/d1*100:+.1f}%" if (d1 and d2) else "?"
            print(f"== {name}: 0.7.9 {u1}/{d1}  ->  0.7.11 {u2}/{d2}   (up {du}, down {dd})")
