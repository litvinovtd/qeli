...
Сложного там оказалось предостаточно. И началось всё скромно — с той самой задачи: чтобы серверы видели друг друга, и не более того. Но аппетит приходит во время еды: как только это заработало, захотелось дотянуть туннель до телефона, потом до рабочего ноутбука, а в какой-то момент я поймал себя на том, что собираю уже не «связку для парка серверов», а полноценный VPN под все платформы. Так у проекта и появились нативные клиенты под Android, Windows и macOS. Теперь это рабочий кросс-платформенный туннель — ядро на Rust, — который на быстром стенде выдаёт под 700 Мбит/с, в дампе выглядит как обычный HTTPS — в сильнейшем режиме неотличимый от настоящего TLS 1.3 для пассивного наблюдателя, — и в котором я наконец понимаю каждый байт. Проект называется **Qeli**, исходники открыты. Это не маркетинг и не «смотрите какой я молодец», а честный технический разбор: архитектура, криптография, набор транспортных режимов и, главное, те места, где я набил шишки. Шишек будет много — без них статья была бы враньём.
...
А в мыслях — ещё ворох допилов, которых в обычном VPN обычно не встретишь; почти все про то, чтобы туннель вписывался в уже существующую инфраструктуру, а не жил отдельным островом:
...
- **тонкий ACL по подсетям и портам назначения** для пользователей и групп — поверх уже существующего разделения по профилям (кому какой профиль → его порт и набор маршрутов): настоящая микросегментация вместо «все в одну плоскую сеть»;
- **метрики Prometheus, вебхуки на connect/disconnect и management API** — чтобы VPN был виден в общем мониторинге и заводился из CI/IaC, а не кликами.

Каждый пункт — это по сути ответ на вопрос «а как поженить туннель с тем, что уже крутится в проде».
