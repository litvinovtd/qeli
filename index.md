...
И ещё, чтобы снять вопрос заранее: проект большой, и да — в работе я активно пользовался ИИ. Как ускорителем на бойлерплейте, кросс-платформенном клее, черновиках доков и как вторым взглядом на ревью. Но это инструмент, а не автор: архитектуру, протокол и каждое спорное решение я принимал сам, отлаживал тоже сам — ИИ не словит за тебя cancellation-safety баг в `select!` и не сверит key schedule с тестовыми векторами RFC 8448. Собственно, поэтому статья и получилась про шишки и «почему так», а не про «смотрите, что сгенерировалось».
