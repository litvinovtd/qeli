...
Началось всё с прозаичной задачи. Для личных и рабочих нужд у меня накопился целый парк серверов, и их надо было связать в одну приватную сеть. Я перепробовал готовые решения и каждый раз упирался в одно и то же: на моих каналах большинство протоколов вели себя нестабильно, а лечение сводилось к тому, чтобы поднять прокси и натянуть поверх него TUN через сторонний костыль. Связывать инфраструктуру стопкой подпорок не хотелось. И в какой-то момент сработал знакомый каждому инженеру самообман: «да я сам напишу, там же ничего сложного» — так и было принято решение собрать продукт, который закрывал бы мои собственные потребности целиком.

Сложного там оказалось предостаточно. И началось всё скромно — с той самой задачи: чтобы серверы видели друг друга, и не более того. Но аппетит приходит во время еды: как только это заработало, захотелось дотянуть туннель до телефона, потом до рабочего ноутбука, а в какой-то момент я поймал себя на том, что собираю уже не «связку для парка серверов», а полноценный VPN под все платформы. Так у проекта и появились нативные клиенты под Android, Windows и macOS. Теперь это рабочий кросс-платформенный туннель — ядро на Rust, — который на быстром стенде выдаёт под 700 Мбит/с, в дампе выглядит как обычный HTTPS, и в котором я наконец понимаю каждый байт, уходящий в канал. Проект называется **Qeli**, исходники открыты. Это не маркетинг и не «смотрите какой я молодец», а честный технический разбор: архитектура, криптография, набор транспортных режимов и, главное, те места, где я набил шишки. Шишек будет много — без них статья была бы враньём.
...
Конфигурация — плоский INI с профилями. Я начинал с JSON, был период TOML, и в итоге выпилил оба: для конфига сетевой службы плоский ключ-значение читается человеком лучше всего, а round-trip через веб-панель проще. Управление — панель на `axum`: дашборд, пользователи, живые сессии, выдача профилей через `qeli://`-ссылки и QR.
...
![Транспортные режимы: одно ядро, разные обёртки](habr-assets/02-transport-modes.svg)
...
**`fake-tls`** — тот же поток, обёрнутый в форму TLS-записей (заголовок application-data `0x17 0x03 0x03`), а рукопожатие открывается ClientHello с GREASE и рандомизированным порядком расширений, так что отпечаток варьируется от сессии к сессии. На уровне фрейминга это обычная TLS-сессия.
...
![reality-tls: рукопожатие и заём сертификата](habr-assets/03-reality-handshake.svg)
...
![Пропускная способность по режимам](habr-assets/04-performance.svg)
...
Управление и клиентов я старался сделать так, чтобы не приходилось лезть в конфиги руками. Пара скриншотов для наглядности.

**Веб-панель администратора** — `axum` плюс Alpine.js, без тяжёлого фронтенда: дашборд, пользователи и группы, живые сессии, редактор профилей и выдача `qeli://`-ссылок с QR в одном месте.

![Веб-панель администратора](habr-assets/panel-dashboard.svg)
*Дашборд: пользователи и группы, живые сессии, выдача профилей через `qeli://` и QR.*

**Десктоп-клиент (Windows / macOS)** — один и тот же C#-интерфейс на обеих ОС, светлая и тёмная темы. Список профилей, статус соединения, графики приёма/отдачи и журнал.

![Десктоп-клиент: главный экран](habr-assets/client-desktop-main.svg)
*Главный экран десктоп-клиента.*

Редактор профиля — те самые поля, что разбирались выше: режим подключения, SNI для маскировки домена, пиннинг ключа сервера, `MTU = auto` (берётся с сервера), маршрутизация «весь трафик» и DNS-резолверы.

![Десктоп-клиент: редактор профиля](habr-assets/client-desktop-editor.svg)
*Редактор профиля: SNI, пиннинг ключа, MTU=auto, full-tunnel, DNS — всё в одном окне.*

**Android** — нативный клиент на Kotlin. На экране подключения виден живой лог рукопожатия: ClientHello, проверка идентичности сервера, `Auth OK`, выданный IP туннеля.

![Android-клиент: подключение](habr-assets/client-android-connected.svg)
*Android: экран подключения с логом рукопожатия.*
...
![Конструктор из кубиков против одного процесса](habr-assets/01-architecture.svg)
...
Проект живой и открытый — Rust плюс Kotlin/C#; ядро и сервер под AGPL-3.0, нативные клиенты под MPL-2.0, исходники на GitHub: [github.com/litvinovtd/qeli](https://github.com/litvinovtd/qeli). Писать свой туннель с нуля — занятие отрезвляющее: начинаешь с «там же ничего сложного», а заканчиваешь побайтовой сверкой с тестовыми векторами. Но это, пожалуй, лучшее, что я сделал руками.
