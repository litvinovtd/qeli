...
Теперь — обещанные числа: аккуратный замер я наконец сделал. Гонял на эмулированном канале (`tc netem` — задержка плюс потери), потому что именно там бондинг и должен себя проявить: на чистой лабе TCP-over-TCP не деградирует, и агрегировать попросту нечего. Тест сразу подсветил тонкость реализации — выигрыш появляется только при нескольких параллельных потоках. Каждый внутренний поток пиннится к одному соединению по flow-хэшу (чтобы не вносить переупорядочивание, от которого внутреннему TCP только хуже), так что один-единственный поток быстрее не станет; а вот десяток одновременных соединений, как у браузера, аккуратно размазывается по бондингу. На восьми параллельных потоках и канале RTT 80 мс / потери 0.1% один туннель отдаёт ~50–65 Мбит/с на скачивание, а четыре склеенных — ~260–305: примерно **пятикратная** разница, устойчивая между прогонами. На чистом канале — паритет, там бондинг и не нужен (упор всё равно в TUN-помпу). Лекарство ровно от того, ради чего задумано — от плохого канала. Механизм задеплоен на всех TCP-режимах (Rust + Android; Win/Mac пока только компилируются).
