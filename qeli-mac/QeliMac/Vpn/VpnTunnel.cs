...
/// <summary>
/// The qeli data plane for macOS. Direct port of the Android QeliService / qeli-win
/// VpnTunnel: shared transport-agnostic handshake + tunnel loop over a small Transport
/// abstraction (TCP or UDP/QUIC), feeding a utun interface. Runs on background threads
/// and raises events the Avalonia UI marshals to the dispatcher.
/// </summary>
public sealed class VpnTunnel
{
    ...
    /// <summary>Headless test: connect + full handshake only (no TUN, no root), return the
    ...
    private void CloseTransports()
    {
        ...
        try { _utun?.Dispose(); } catch { }
        _tcp = null; _udp = null; _obfs = null; _net = null; _utun = null;
    }
    ...
    private void ConnectWithRetry(VpnConfig config, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            catch (System.Security.SecurityException e) when (!ct.IsCancellationRequested)
            {
                ...
                // retry; surface a clear security warning and stop. (A5 — TOFU warning.)
                Log($"[SECURITY] {e.Message}");
                Status(VpnStatus.Error, "Server identity changed — possible MITM. Connection stopped.");
                CloseTransports();
                break;
            }
    ...
    private void SetupTun(VpnConfig config, Session session, IPAddress serverIp)
    {
        ...
        _utun = new UtunDevice();
        _utun.Open();
        string dev = _utun.Name;
        ...
        PacketCodec enc, PacketCodec dec, bool isUdp, CancellationToken ct)
    {
        ...
        // Upload: system -> tunnel (read utun outbound packets, encrypt, send).
        ...
        // Download: tunnel -> system (recv record, decrypt, inject into utun).
        var downloadJob = Task.Run(() =>
        {
            try
            {
                while (!ct.IsCancellationRequested)
                {
                    ...
                    {
                        utun.SendPacket(plaintext, plaintext.Length);
