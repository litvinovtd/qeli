...
/// <c>realtls</c> core via P/Invoke over <c>libqeli.dylib</c> (the C ABI in
/// <c>src/protocol/realtls/ffi.rs</c>). The same genuine browser-grade TLS stack
/// the Rust, Android and Windows clients use, so the macOS client's on-wire
/// fingerprint matches Chrome. The qeli tunnel runs <i>inside</i> this TLS
/// session (nested). The dylib ships in the .app bundle next to the executable,
/// so the default DllImport search finds it.
/// </summary>
public sealed class RealTls : IDisposable
{
    private const string Dll = "qeli"; // libqeli.dylib, in the .app bundle next to QeliMac
