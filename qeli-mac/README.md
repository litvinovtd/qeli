...
(`qeli-win`, C#/WPF) на **C# / .NET 8 + Avalonia UI**. Полностью повторяет протокол:
...
| GUI                   | Avalonia UI 11 (.NET 8) — кросс-платформенный аналог WPF             |
...
- **.NET 8 SDK** — публикует self-contained payload (`dotnet publish -r osx-arm64`).
...
xattr -dr com.apple.quarantine Qeli.app    # снять карантин при копировании
open Qeli.app                               # GUI
```
...
1. **GUI под sudo** — приложение целиком работает от root:
   ```bash
   sudo "dist/Qeli.app/Contents/MacOS/QeliMac"
   ```
   (Запуск двойным кликом / `open Qeli.app` работает, но при нажатии «Подключить» без
   root появится подсказка — данные-плейн нуждается в правах.)
2. **launchd-демон** — в **Настройках** включите «Запускать как демон launchd» и
   выберите профиль. Демон ставится в `/Library/LaunchDaemons`, работает от root,
   стартует при загрузке (до входа) и сам переподключается; GUI тогда лишь показывает
   статус/журнал и кнопкой управляет демоном. Установка демона требует, чтобы GUI был
   запущен от root (sudo).
