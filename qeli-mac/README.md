...
| Transport/crypto      | Rust `libqeli.dylib`, ABI 1.14 (`qeli_client_run` + native utun fd)  |
...
│   ├── Vpn/           UtunDevice lifecycle, NetworkConfigurator, ABI 1.14 adapter
...
Rust-ядро ABI 1.14. Невыбранные потоки остаются на системном маршруте/DNS. Во время reconnect
...
| Whole-client `qeli.dll` (ABI 1.14)       | `libqeli.dylib` (universal, тот же ABI 1.14)      |
