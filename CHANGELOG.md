...
- Рамка распознавания QR на Android теперь рассчитывается после окончательного размера диалога
  и центрируется вместе с реальной областью декодирования; поворот устройства пересоздаёт scanner
