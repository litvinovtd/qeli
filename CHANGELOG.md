...
- Android, iOS, Windows, macOS и Linux обрабатывают события через общее Rust-ядро ABI 1.15.
  ...
  `reconnect_allowed=false` запрещает reconnect. TCP использует упорядоченную очередь carrier,
  UDP трижды передаёт один message ID, а bounded reassembler дедуплицирует повторы.
...
- Панель исключает выключенные профили из runtime-сводки роуминга, поэтому они больше не отображаются жёлтыми как «нет данных процесса». Навигация профилей конфигурации перенесена с горизонтальной прокрутки на адаптивную компактную сетку, а транспорт обозначается явными текстовыми badges `TCP`/`UDP`.
...
- Все выпадающие списки панели получили единое центрирование и симметричные отступы под стрелку. Компактные фильтры автоматически измеряют самый длинный пункт после локализации и динамического наполнения, поэтому русские подписи больше не обрезаются шириной, подобранной под английский текст; gate запрещает возвращать фиксированную ширину select-полей.
