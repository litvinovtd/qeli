...
- Карточка «Нагрузка хоста» на dashboard переработана в единый адаптивный операционный блок:
  состояние метрик, CPU/RAM, load average, память и диск, процесс qeli, WAN, TCP/UDP и аптайм сгруппированы по смыслу; предупреждения используют те же пороги 90%, что и системный status API, а компоновка перестраивается по ширине самой карточки.
