...
### Добавлено — панель за reverse-proxy на сабпасе
- **`web.base_path`** — панель теперь можно отдавать не в корне домена, а под префиксом
  (например `https://host/qeli/`). Ядро вкладывает роутер под префикс, вставляет `<base href>`
  и дописывает префикс к редиректам; учитывает заголовок `X-Forwarded-Prefix`. Пусто (по
  умолчанию) = как раньше, в корне. См. «Reverse-proxy sub-path» в `CONFIG.md` / `PANEL.md`.
- **`web.csrf`** (default `true`) — тумблер CSRF-защиты панели. `false` полностью
  отключает проверку Origin/Referer (со стартовым предупреждением) — допустимо только на
  loopback-bind; для форвардинга это не нужно (loopback-origin и так доверяется на любом порту).

### Изменено
- **Понятный 403 при отклонении CSRF** — вместо немого запрета панель отдаёт `text/plain`
  с именем отклонённого origin и готовым примером, что добавить в `web.allowed_origins`.
...
### Документация
- **Клиентский `dns`.** Описание расширено: явно сказано, что дефолт `tunnel` **переписывает
  `/etc/resolv.conf`** (Linux) ради анти-DNS-leak, а `dns = off` — не трогает системный резолвер
  и использует DNS хоста как есть (не только для роутеров, но и для любого Linux-хоста с уже
  настроенным DNS). `CONFIG.md` ru+eng, включая коммент в примере роутер-конфига.
