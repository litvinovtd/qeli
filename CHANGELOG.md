...
- Активная проверка UDP-профиля на iOS теперь всегда использует нативный UDP first flight, в том
  ...
  TCP connect. Android ограничивает одновременно выполняемые ручные и автоматические проверки
  четырьмя слотами, поэтому большой список профилей не создаёт burst из DNS-запросов и сокетов.
- Android читает импортируемые профили и backup вне UI thread с пределом 8 МиБ. До замены текущего
  набора проверяются число профилей, active index, размер имён/конфигов и полная валидность каждого
  INI; ошибка в середине архива больше не оставляет частично восстановленный набор. Ошибка формата
  расшифрованного backup показывается отдельно от неверной парольной фразы.
