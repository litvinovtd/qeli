...
- UDP datapath на Linux и Android получает до 32 уже ожидающих датаграмм одним `recvmmsg` на
  ...
  лабе не подтвердил прирост goodput: относительно предыдущего бинарника результат составил −1,5%
  upload и −3,7% download, что сопоставимо с разбросом соседних TCP-окон. При этом CPU принимающего
  процесса qeli снизился примерно на 10–12%; поэтому изменение фиксируется как снижение
  syscall/CPU-нагрузки и дополнительный запас обработки, а не как обещание роста скорости.
