...
- Исправлен desktop `route_file`: ключ можно повторять для нескольких файлов, поддерживаются
  как CIDR, так и OpenVPN `route ... netmask ...`/`route-ipv6` строки, сети канонизируются и
  дедуплицируются, а неверный или недоступный файл останавливает подключение fail-closed.
  Установка больших списков Windows/macOS теперь отменяется кнопкой отключения и пишет один
  итоговый log вместо тысяч синхронных UI-сообщений. Windows направляет импортированные сети
  через аутентифицированный tunnel gateway, поэтому для каждой подсети больше не появляется
  дополнительный broadcast `/32`; системные host routes самого tunnel pool сохранены.
