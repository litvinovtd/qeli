...
- Панель конфигурации: оставлены форма и INI-редактор; JSON-редактор удалён. Формат файлов
  остаётся только INI, JSON используется для служебного обмена с API.
- Исправлены гонки сохранения, обработка ошибок загрузки и требование полного перезапуска
  для настроек web listener. Worker restart теперь отклоняет такие изменения на сервере.
- Строгие входы отклоняют повторные секции профилей/пользователей/групп, неизвестные поля
  данных формы и неправильные типы списков доступа. Запятые в именах профилей запрещены,
  поскольку эти имена используются в списках ACL; существующие такие имена нужно переименовать.
- INI сохраняет значимые пробельные символы и кавычки; форматирование заголовков не стирает
  замаскированные секреты. Клиентская форма правильно читает boolean и удалённые raw-поля.
  Изменение квоты сохраняет точный срок доступа, если дата не редактировалась.
- Добавлены исполняемые регрессии редакторов панели и проверки parser/API-границ.

- Закрыты подтверждённые замечания аудита транспорта: HTTP/2 carrier владеет своими
  задачами и освобождает их при Drop/отмене handshake; отправка использует частичное
  flow-control окно, а bonded rate limiter учитывает весь общий долг без обрезки ожидания.
- Windows: проверка службы и native-кэша отклоняет неизвестных владельцев/писателей,
  reparse points и нечитаемые ACL. Отказ защищённого извлечения DLL больше не запускает
  стандартный поиск. Сервисный профиль проверяется перед чтением и публикуется атомарно;
  ошибки защиты, повреждения и незавершённой очистки сети не маскируются успешным статусом.
  Старое небезопасное хранилище не принимается автоматически: требуется явная
  перенастройка из elevated GUI после сохранения нужных пользовательских данных.
- Managed-конфиги Windows/macOS строго проверяют IP/CIDR и размеры сокетных буферов;
  добавлен общий положительный/отрицательный corpus для C# и Rust.
- Панель: хеширование паролей использует общий с VPN-аутентификацией бюджет Argon2,
  удерживаемый самой blocking-задачей при отмене запроса. Проверка бэкапа потоковая:
  ограничены число записей, распакованный размер, длина строки, stderr и время работы tar/gzip.
- REALITY-TLS ограничивает один traffic key в каждом направлении до 2^24 записей
  или 64 GiB ciphertext. При исчерпании соединение завершается для переподключения;
  KeyUpdate по-прежнему не поддерживается.
- Локальный Linux CI собирает release с обязательным jemalloc. Вспомогательный SSH-скрипт
  использует ключи/agent, проверяет known_hosts и применяет deadline при одновременном чтении stdout/stderr.
