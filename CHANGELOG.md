...
- Android release-сборка теперь синхронизирует общие `conformance/*.json` на лабу перед unit tests, а fixtures явно перечисляют Kotlin как обязательного потребителя. APK больше не может быть сертифицирован по устаревшим векторам от другого коммита.
