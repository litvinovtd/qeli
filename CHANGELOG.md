...
- Quick Start теперь перебирает все 69 888 private `/24` из RFC 1918 (`10/8`, `172.16/12` и
  `192.168/16`), поэтому маршрут хоста на весь `10/8` не блокирует автоматическое создание
  профиля. Генератор, устанавливаемые примеры и RU/EN документация больше не дублируют
  автоматически зарезервированный `tun.address` в `pool.exclude`.
