...
### Windows — per-app split tunnel (WinDivert)

- На Windows добавлен раздельный туннель по приложениям (`apps_mode` / `apps`): пути к
  `.exe` выбираются в UI, трафик классифицируется через WinDivert на NETWORK-слое.
  Режим `include` — fail-closed: неизвестный владелец пакета и обрыв VPN не отпускают
  выбранное приложение в обход туннеля.
- Data plane хранит полноценную таблицу потоков (исходный IP, IfIdx, протокол, порты,
  DNS-состояние с TTL). Учтены IP-фрагменты и коллизии UDP-портов на разных интерфейсах.
- Фильтр WinDivert перехватывает IPv4 и IPv6; при `allow_ipv6_leak = false` IPv6
  туннелируемых приложений отбрасывается (сервер IPv4-only). RFC1918 не уходит напрямую безусловно: учитываются `route_local`, пользовательские `include`/`exclude` и server-pushed маршруты.
- Elevated-запуск извлекает `wintun`/`qeli`/`WinDivert` в
  `%ProgramData%\QeliWin\native` с проверенным DACL (не в `%LOCALAPPDATA%`).
- Добавлены NOTICE/LICENSE и provenance для WinDivert 2.2.2 в
  `native-libs/third-party/windows-x64/windivert/`; `selftest` проверяет data-plane
  unit’ы, `windivert-e2e` — elevated smoke (полный lab e2e описан в `qeli-win/README`).
...
_Известный остаток, намеренно не замазанный:_ если хук ссылается на скрипт, лежащий внутри
...
- **Windows + macOS (общий `VpnTunnelBase`), только при `persist_tun = true`.** Во-первых,
  ...
  _до или во время_ `SetupTun` тоже «сохранялся», пропуская `CleanupPlatform()` — единственное
...
(изоляция отбрасывает пакет, чей источник — _другой клиентский_ IP, поэтому подделка под
...
- **Rust UDP-uplink** капает `data+padding` по _обнаруженному_ MTU (было: литерал `1400` — при узком
...
- **Редактор политики блокировки на вкладке «Blocked IPs».** Карточка _Lockout policy_ с
