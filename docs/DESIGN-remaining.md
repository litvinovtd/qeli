...
**Как будет работать.** В рукопожатии — оба обмена:
- клиент шлёт X25519 `key_share` + ML-KEM-768 public key;
- сервер отвечает X25519 share + ML-KEM ciphertext (инкапсуляция);
- общий секрет: `HKDF(x25519_shared ‖ mlkem_shared)` → AEAD-ключи.
Ломается только если сломаны *оба*. Бонус: ML-KEM-768 pubkey ~1184 Б / ct ~1088 Б —
ClientHello раздувается (плюс к анти-амплификации, минус к size-fingerprint).

**Что сделать.**
- Rust: крейт `ml-kem` (или `pqcrypto-kyber`); расширить `crypto/exchange.rs` +
  рукопожатие (`handler.rs`/`client`) — новый extension с ML-KEM-материалом; HKDF
  склеивает два секрета.
- Android: BouncyCastle (есть Kyber/ML-KEM) — зеркало в `crypto/KeyExchange.kt`.
- qeli-win: BouncyCastle.Cryptography (есть ML-KEM) — `Crypto/KeyExchange.cs`.
- Версионирование рукопожатия (старый клиент ↔ новый сервер): negotiation-флаг.

**Объём: средне-крупно** (крипто-чувствительно, ×3 клиента, версионирование).
**Гармонирует с Осью 1**: если переходим на настоящий TLS 1.3, гибрид KEX — это
штатный `X25519MLKEM768` группы в реальном стеке, т.е. делать вместе с Осью 1.
