...
```ini
[roaming]
enabled = true            # включить роуминг (UDP-миграция + TCP grace/JOIN-resume)
grace_secs = 30           # сколько TCP-сессия живёт с 0 стримов под JOIN-resume
cid_rotation = true       # ротация UDP roam-CID (анти-линкуемость) — НЕ выключать без причины
max_orphaned = 256        # потолок одновременно «осиротевших» сессий на профиль (анти-DoS)
path_validation = false   # (Фаза 2) QUIC-style challenge нового адреса
```
