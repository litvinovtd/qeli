...
выполнения. **Это план — код пока не трогаем.**
...
**Затронуто:** ~97 из 102 скриптов в `scripts/` (используют `paramiko`); хосты `10.66.116.10` (×97 вхождений), `.11` (×69), прод `222.167.246.143` (×43).
...
- Константы хостов: `LAB_SRV = ("10.66.116.10", "root")`, `LAB_CLI = ("10.66.116.11", "root")`, `PROD = ("222.167.246.143", "root")`; пароль — **из env** (`QELI_LAB_PASS`), как уже делают `e2e_*`-скрипты.
