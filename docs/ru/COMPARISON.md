...
| **qeli reality-tls** | ~550 ↑ / ~430 ↓ Mbps | ~32% | ~2.0 ms | настоящий TLS внутри; ↓ ниже (двойной AEAD на клиенте) |
...
| V2Ray / Xray | Большое сообщество, зрелость, экосистема клиентов под всё. (По REALITY-сертификату qeli достиг паритета — cert-borrowing, см. ниже.) |
| Shadowsocks | Минимум ресурсов, бегает на роутерах. |

По **активному DPI**: у qeli два уровня REALITY. `reality` (proxy) *проксирует*
чужие хендшейки на реальный сайт (`target`, напр. microsoft — пробник видит его),
но qeli-клиент всё ещё шлёт fake-TLS. **`reality-tls` (готов)** — клиент шлёт **настоящий** браузерный
 TLS 1.3 (Chrome JA4), сервер терминирует его и несёт туннель внутри. Он закрывает
 известные теллы 1.1–1.6 из DPI-AUDIT, но этот результат не покрывает универсальный
 анализ таймингов, размеров записей и корреляции трафика и не является гарантией неотличимости.
**Cert-borrowing (`handrolled=true`, 2026-06-06) закрыл прежний разрыв с
Xray-REALITY:** hand-rolled сервер при старте **одалживает настоящую цепочку серта
...
REALITY (qeli достиг паритета по reality-tls + cert-borrowing + PQ-гибрид, но менее
...
| TLS-маскировка | ✘ | реальный TLS | реальный TLS + REALITY | fake-TLS + настоящий TLS (reality-tls) |
| Встроенная админка | ✘ | ✘ | ✘ | ✅ |
