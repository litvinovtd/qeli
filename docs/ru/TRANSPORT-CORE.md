...
транспортов. Socket/codec остаются в прежнем wire-коде, поэтому формат провода не изменён.
...
| TC-1.2 | Data-plane путь **без аллокаций на пакет**: буферы вызывающей стороны, никаких `Box::into_raw` на горячем пути | 🟦 Linux TUN fd/workers/очереди вынесены в ядро; переиспользуемые буферы и внешний FFI-шов впереди |
...
полный lab build зелёный (509 библиотечных тестов), netns routing/kill-switch e2e — 26/26,
TCP fake-TLS — 612↑/715↓ Мбит/с, TCP obfs — 623↑/629↓ Мбит/с, UDP — 400 Мбит/с при
0,24% потерь. Весь TC-1 ещё не закрыт: wire socket/handshake/codec остаются в старом модуле,
а очереди TUN пока передают отдельный `Vec` на пакет. Переиспользование буферов и перенос
владения транспортом — следующий этап TC-1.2.
