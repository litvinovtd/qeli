...
Отложено осознанно. Текущий контракт теперь явный и fail-fast: общий Rust-транспорт и
shared-валидация десктопов отвергают IPv6-литерал сервера, installer отвергает IPv6
`PUBLIC_HOST`, а мобильные адаптеры используют то же Rust-ядро. У hostname должна быть
A-record. Это заявленная отсутствующая возможность, а не прежний поздний connect-time отказ.
...
Затем валидация отказывает: синтаксис понятен для будущего совместимого формата, но не принят.
...
- **Rust всё ещё собирает два runtime-адреса без скобок**, тогда как основной TCP-resolver
  безопасно принимает `(host, port)`. Пути нужно свести к одному bracket-aware helper.
- **Выбор TCP-кандидатов явно оставляет только IPv4**, поэтому hostname только с AAAA
  отказывает ещё до создания сокета; нужно определить dual-stack ordering/fallback.
- **UDP-плоскость Rust биндит `0.0.0.0:0`** — только IPv4, что бы ни стояло в эндпоинте.
- **Диагностика доступности десктопов оставляет только A-record.** Production-транспорт
  Windows/macOS/iOS/Android уже общий Rust, но platform validation, bypass routes и
  диагностику всё равно нужно синхронно сделать dual-stack.
...
- ✅ **FFI panic-safety: клиентские cdylib собираются с `panic = "unwind"`** (завершено в
  0.7.15, [TRANSPORT-CORE TC-0.1](TRANSPORT-CORE.md)). Native-рецепты Android/Windows/macOS/iOS
