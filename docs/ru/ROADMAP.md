...
- 🔵 **Continuous fuzzing в CI** — nightly-прогон таргетов `qeli/fuzz/` + корпус;
  начато в 0.7.2 (харнес есть, прогон ручной).
...
10. 🔵 **Многоядерный data-plane** — сейчас направление TUN→клиент сериализовано в
    одной задаче с inline-шифрованием (прод-потолок ~311 Мбит, 1-ядро-bound). Шардить
    fan-out по ядрам/очередям без единой точки сериализации; мерить на multi-core.
