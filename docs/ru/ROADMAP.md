...
10. ⚪ **Многоядерный data-plane — НЕ планируется (измерено 2026-06-19: упор не в CPU).**
    ...
    очередь + один codec (монотонный счётчик → nonce) = 1 ядро. **Замер:** прод =
    **1 vCPU** (распараллеливать физически некуда); потолки (прод ~100–311 / лаб
    ~590 Мбит) — **network/VM-bound**, не шифратор (AES-NI ~8 Гбит/с/ядро, single-flow
    qeli ≤ ~0.8 ядра). Распараллеливание одного потока = наивысший риск (nonce-uniqueness
    в горячем пути под `panic="abort"`) при нулевой выгоде. **Рычаг для throughput =
    более ёмкий VM + широкий аплинк, не код.** Закрыто.
