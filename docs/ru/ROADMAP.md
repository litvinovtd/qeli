...
### Остатки разбора «после сна падает скорость» (2026-08-15)

Разбор дал две правки — политика пробуждения и размер слотов downlink-пула, — но
одинаковые дефекты остались ещё в двух местах. Оба того же класса: **намеренный** цикл
переподключения засчитывается в счётчик неудач, и **резерв буфера считается от
протокольного максимума, а не от реального размера записи**.

- **Rust-клиент (CLI): намеренный реконнект наращивает задержку.** `retry_count`
  обнуляется только после 30 с стабильной сессии, а выход из спящего режима
  (`resumed from suspend — reconnecting`, TCP и UDP пути) рвёт сессию сам. Если сон
  случился раньше 30 с после подключения, пауза растёт 1→2→4→…, как это было на Android.
  Лечится так же: пометить цикл как намеренный и не эскалировать. **Не сделано только
  потому, что в `client/mod.rs` шла параллельная правка** — вносить туда ещё изменения
  значило гарантировать конфликт.
- **iOS/packet-мост (`transport_core/packet_tun.rs`): пул из 32-64 слотов по 64 КБ.**
  Тот же перерасход, что был на Android (251 слот) и Windows (64 слота), только резерв
  ещё крупнее. Требует протащить согласованный MTU через ядро до `PacketTunPump::new`,
  поэтому дороже двух уже сделанных правок; отложено ещё и потому, что iOS сейчас не
  собирается.
- **Телеметрия потерь свалена в один счётчик.** `internal_drops` смешивает пять разных
  причин (пул исчерпан, очередь полна, пакет крупнее MTU, не-IPv4, ошибка записи в TUN).
  Из-за этого одна и та же цифра в логе допускала взаимоисключающие трактовки. Разнести
  на отдельные счётчики. Отдельно: дропы самого писателя TUN при `EAGAIN/ENOBUFS`
  в `internal_drops` вообще не попадают — их не видно ни в логе, ни в статистике.
