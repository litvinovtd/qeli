...
Пустой `dns` = клиент оставляет свои резолверы. Дефолтный `dns.listen` (`10.0.0.1`) пушится
...
Ссылка `qeli://` несёт ровно: `host:port`, `user`, `pass`, `proto`, `mode`, `key`, `sni`,
`reality_sid`, `obfs_key`, `front`, `quic`, `mtu=0`, `awg`/`jc`/`jmin`/`jmax` и метку —
то есть **только то, что клиент не может узнать иначе**. Маршрутов и DNS в ней нет by design.
...
persist_session_key = true    # логины в панель переживают перезапуск; эмитится только при false
