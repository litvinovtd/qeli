...
- **Проверка обновлений (баннер новой версии):** панель может показывать
  ...
  > ⚠️ Флаг `[web] update_check` существует как поле структуры `WebConfig`, но **не
  > сериализуется и не парсится** INI-кодеком (`web_to`/`web_from` в `server_ini.rs`),
  > поэтому в flat-INI это **нерабочий ключ** — прописывать `update_check =` в `[web]`
  > эффекта не даёт.
