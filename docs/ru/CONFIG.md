...
- Клиентский конфиг — одна секция `[qeli]`; это же разворачивается из `qeli://`-ссылки
  ...
  обе стороны обязаны совпадать по числу junk-пакетов) с `jc`/`jmin`/`jmax` (число junk-пакетов
  и его мин/макс размер); должны совпадать с серверными `obf.awg.*` (см. раздел обфускации),
...
| `dns` (дефолт `tunnel`) | режим клиентского DNS; `off` = не трогать системный резолвер (для роутера). File-only; эмитится в INI только при `!= tunnel` |
...
| `obf.awg.enabled` | `false` | AmneziaWG-подобный junk перед рукопожатием: шлёт `jc` случайных «junk»-пакетов до настоящего рукопожатия, чтобы первые байты на проводе не несли фиксированной сигнатуры. **Обе стороны обязаны совпадать по `jc`** — приёмник пропускает ровно столько junk-пакетов; рассинхрон ломает рукопожатие. Клиентская сторона: `awg`/`jc`/`jmin`/`jmax` в `[qeli]` / `qeli://` |
| `obf.awg.jc` | `0` | число junk-пакетов перед рукопожатием (`0` = нет; зажато сверху `128`) |
| `obf.awg.jmin` / `jmax` | `40` / `300` | диапазон размера junk-пакета в байтах (`jmin ≤ jmax ≤ 1400`) |
