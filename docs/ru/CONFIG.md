...
> **Редактируются в панели** (Config → Global: «Failed logins before lockout / window /
> lockout duration») — не только в файле. Применяются кнопкой **Apply & Restart** либо
> `SIGHUP`-reload: сервер пересобирает трекер с новыми значениями (in-flight счётчики
> локаутов при этом сбрасываются). Тот же локаут защищает и **вход в панель**.
> Внутренние задержки tarpit'а (200 мс … 3 с) не настраиваются. Заблокированные адреса —
> вкладка **«Blocked IPs»** / `qeli list-blocked` (см. [PANEL.md](PANEL.md),
> [GETTING-STARTED.md](GETTING-STARTED.md) §10).
>
> Правьте три порога в панели: **Config → Authentication → Brute-force Protection**
> (редактируемая карточка, что была на вкладке *Blocked IPs*, убрана в 0.7.7 — теперь та
> вкладка просто ссылается сюда). Одна политика управляет обеими плоскостями (вход в
> панель + аутентификация VPN).
