...
>
> Все три можно править и прямо на вкладке **«Blocked IPs»** (карточка *Lockout policy*):
> сохранение патчит **только эти ключи, на месте** (комментарии сохраняются) и применяет
> их **на лету** через `SIGHUP` — без рестарта, активные сессии не рвутся. Одна политика,
> обе плоскости (вход в панель + аутентификация VPN).
