...
| `dns_servers` | A→A | A→A | C→A | C→A | C→A | Все клиенты используют канонический список `dns_servers`; мобильные всё ещё импортируют старое `dns = IP, IP`, но сохраняют канонический вид. Публичный fallback DNS не подставляется. |
