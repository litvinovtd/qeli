...
`file` (путь к лог-файлу; по умолчанию stderr → journald), `format`.

> ⚠️ **`format = json` — заглушка.** Поле парсится и показывается в панели, но
> `init_logging` его **не читает** — лог всегда в plain-формате
> `YYYY-MM-DD HH:MM:SS:mmm LEVEL target: сообщение`. Не рассчитывайте на JSON-логи.
...
**Журнал** в окне приложения. Каждая строка с UTC-таймстампом
`2026-07-11T19:02:41Z  …`. Кнопки **Copy log** / **Clear log** в шапке журнала.
...
Autoscroll, буфер 500 строк, префикс `[HH:mm:ss]`. Плюс через `adb`:
```bash
...
подключиться **с другой сети** (проводной Ethernet 1500) — если там работает,
причина 100% в MTU. На клиенте можно снизить `mtu` в профиле.
...
ServerHello, лечение утечки writer'ов). **Фикс:** обновить сервер до 0.7.11-dev и
