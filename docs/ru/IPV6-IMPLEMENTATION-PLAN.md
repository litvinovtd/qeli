...
Это ещё не утверждение о готовности релиза: перед выпуском должна пройти физическая/native-
матрица и Linux network namespace-матрица из раздела 14 с заново собранными ядрами клиентов.
...
проверяет помеченный профилем DROP для egress не через этот TUN, чтобы профиль не унаследовал
forwarding от другого активного профиля или глобальной настройки хоста. `route` принимает
входящий трафик только в делегированный prefix профиля; `nat66` — только WAN-ответы
related/established.
...
значение. Реализация может использовать nftables или ip6tables, но sysctl/firewall cleanup,
...
- **macOS per-app:** transparent proxy направляет выбранный IPv6 в tunnel, использует
  family-корректные source/destination/interface binds и DNS relay; здесь не нужно копировать
  raw-NAT модель Windows.
...
17. ⏳ Осталось пройти физическую release matrix и пересобрать/сверить все native-ядра.
